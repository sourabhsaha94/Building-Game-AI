...
public class Astar {
	/*
	 * Definition of expanded as mentioned by the homework assignment
	 * corresponds to the Visited arrayList below
	 */
	Graph gh;
	...
	void calculateHcostEuclidian() {
		...
		for (Vertex v : gh.Vlist) {
			v.h_cost = Math.sqrt(Math.pow((69.5 * (goal.lat - v.lat)), 2)
					+ Math.pow(69.5 * Math.cos((goal.lat + v.lat) / 360 * Math.PI) * (goal.lon - v.lon), 2));
		}
	...
	int getIndexOfVertexInPath(Vertex v){
		
		for(int i=0;i<finalPath.size();i++){
			if(gh.calculateDistance(v.lat, v.lon, finalPath.get(i).lat, finalPath.get(i).lon)<2){
				return i; 
			}
		}
	...
	public Vertex quantizeOnPath(float x,float y){
